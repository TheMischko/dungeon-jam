...
export class PlaylistHelper {
  static async getPlaylistsWithoutChildren(playlists: Playlist[]): Promise<Playlist[]> {
  ...
  static async getPlaylistsWithOwnership(playlists: Playlist[]): Promise<Playlist[]> {
    const playlistMap = await this.getPlaylistMap(playlists);
    const updatedPlaylists = PlaylistHelper.addOwnershipToPlaylists(playlists, playlistMap);
  ...
  static addOwnershipToPlaylists(playlists: Playlist[], playlistMap: Map<string, Playlist>, parentId?: string): Playlist[] {
    return playlists.reduce((processed, playlist, _, __) => {
      const updatedPlaylist = { ...playlist, parentId: parentId ?? undefined };
  ...
  static async getPlaylistMap(playlists?: Playlist[]): Promise<Map<string, Playlist>> {
    if(!playlists) {
      return new Map();
    }
    const records = playlists.map(playlist => [playlist.id, playlist] as const);
